...
#
# [*manage_config*]
#   Whether to create and manage the base config.json.  This is useful if other services 
#   such as consul_template manage your config files. Defaults to `true`.
#
# [*manage_group*]
...
#   This is ignored when manage_config == 'false'.  Defaults to `true`.
#
# [*restart_on_change*]
...
class consul (
  ...
  $manage_config         = $::consul::params::manage_config,
  ...
  if $manage_config == false {
    $purge_config_dir_real = false
  }
  else {
    $purge_config_dir_real = $purge_config_dir
  }
