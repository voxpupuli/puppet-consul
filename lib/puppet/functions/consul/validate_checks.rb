    ...
    case obj
      ...
      when Hash
        ...
        if obj.key?("ttl")
          ...
          if ( obj.key?("http") || obj.key?("tcp") || obj.key?("args") || obj.key?("script") || obj.key?("interval") )
            raise Puppet::ParseError.new('alias_service must not define http, tcp, args, script, or interval')
          end
        else
